fn main() {
    ...
    let arg = std::env::args().nth(1).expect("Please pass in a number as a command line argument");// TODO: get the first argument
...
fn uppercase(src: &str) -> String {
    ...
    for c in src.chars() {
        ...
        destination.push(c.to_uppercase().next().expect("failed to convert to upper"));
